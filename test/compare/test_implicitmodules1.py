...
class TestCompareImplicitModules1(unittest.TestCase):
    def setUp(self):
        ...
        self.implicit1_torch = ImplicitModule1.build_and_fill(2, self.N, torch.tensor(self.C), self.sigma, 0.001, gd=(torch.tensor(self.q).view(-1), torch.tensor(self.R).view(-1)), cotan=(torch.tensor(self.p).view(-1), torch.tensor(self.p_R).view(-1)))
