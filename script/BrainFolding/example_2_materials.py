...
Z0_c = np.concatenate(
    [np.array([X0, np.zeros([nx]) + ymin]).transpose(), np.array([np.zeros([ny]) + xmax, Y0]).transpose(),
     np.array([np.flipud(X0), np.zeros([nx]) + ymax]).transpose(),
...
Z1_c = np.concatenate(
    [np.array([X1, np.zeros([nx1]) + ymin1]).transpose(), np.array([np.zeros([ny1]) + xmax1, Y1]).transpose(),
     np.array([np.flipud(X1), np.zeros([nx1]) + ymax1]).transpose(),
