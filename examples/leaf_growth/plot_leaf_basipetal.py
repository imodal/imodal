...
source = imodal.Utilities.close_shape(torch.tensor(data_source[1]).type(torch.get_default_dtype()))
target = imodal.Utilities.close_shape(torch.tensor(data_target[1]).type(torch.get_default_dtype()))
...
plt.plot(shape_source[:, 0].numpy(), shape_source[:, 1].numpy(), color='blue', lw=0.5)
...
plt.plot(shape_target[:, 0].numpy(), shape_target[:, 1].numpy(), color='green', lw=0.5)
plt.axis('equal')
...
model = imodal.Models.RegistrationModel([deformable_shape_source],
            ...
            [imodal.Attachment.VarifoldAttachment(2, [20., 60.])], lam=100.)
...
###############################################################################
# Plot results. Matching is good and growth points follow a basipetal growth pattern.
#
...
plt.plot(points_small[:, 0].numpy(), points_small[:, 1].numpy(), '.', color='red')
plt.plot(points_growth[:, 0].numpy(), points_growth[:, 1].numpy(), '.', color='blue')
