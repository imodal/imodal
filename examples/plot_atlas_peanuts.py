...
sigmas_varifold = [0.5, 2.]
atlas = dm.Models.Atlas(template.clone(), [global_translation, left_scale, right_scale], [dm.Attachment.VarifoldAttachment(2, sigmas_varifold)], len(peanuts), lam=10., optimise_template=False, ht_sigma=0.2, fit_gd=[False, True, True])
...
fitter.fit(peanuts, 150, log_interval=10, options={'shoot_method': shoot_method, 'shoot_it': shoot_it})
...
with torch.autograd.no_grad():
    deformed_templates = atlas.compute_deformed(shoot_method, shoot_it)

row_count = math.ceil(math.sqrt(len(peanuts)))

for i, deformed_template, peanut in zip(range(len(peanuts)), deformed_templates, peanuts):
    plt.subplot(row_count, row_count, 1 + i)
    plt.plot(deformed_template[:, 0].numpy(), deformed_template[:, 1].numpy())
