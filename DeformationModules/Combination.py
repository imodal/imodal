...
from DeformationModules import DeformationModules as defmod
from GeometricalDescriptors import GeometricalDescriptors as GeoDescr
from StructuredFields import StructuredFields as stru
...
class CompoundModules(defmod.DeformationModule):  # tested
    ...
    def init_GD(self):  # tested
        self.GD = GeometricalDescriptors.Combine_GD.Combine_GD([Modi.GD for Modi in self.ModList])
    ...
    def field_generator_curr(self):  # tested0
        return StructuredFields.SummedFields.sum_structured_fields([self.ModList[i].field_generator_curr() for i in range(self.NbMod)])
    
    def field_generator(self, GD, Cont):  # tested
        GDlist = GD.GD_list
        return StructuredFields.SummedFields.sum_structured_fields(
    ...
    def DerCost_curr(self):  # tested0
        ...
        return GeometricalDescriptors.Combine_GD.Combine_GD(derlist)
    ...
    def cot_to_innerprod_curr(self, GDCot, j):  # tested0
        ...
        if j == 1:
            ...
            out = GeometricalDescriptors.Combine_GD.Combine_GD(derlist)
