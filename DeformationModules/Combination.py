...
# -*- coding: utf-8 -*-
"""
Created on Mon Dec  3 10:28:35 2018
...
from StructerdFIelds import StructuredFields as stru
...
class CompoundModules(defmod.DeformationModule):#tested
    ...
    def init_GD(self):#tested
        self.GD = GeoDescr.Combine_GD([Modi.GD for Modi in self.ModList])
    ...
    def field_generator_curr(self):#tested0
        return stru.sum_structured_fields([self.ModList[i].field_generator_curr() for i in range(self.NbMod)])
    
    def field_generator(self, GD, Cont):#tested
        GDlist = GD.GD_list
        return stru.sum_structured_fields([self.ModList[i].field_generator(GDlist[i], Cont[i]) for i in range(self.NbMod)])
    ...
    def DerCost_curr(self):#tested0
        ...
        return GeoDescr.Combine_GD(derlist)
    ...
    def cot_to_innerprod_curr(self, GDCot, j):#tested0
        ...
        if j==1:
            ...
            out = GeoDescr.Combine_GD(derlist)
