...
class Landmarks(Manifold):
    def __init__(self, dim, nb_pts, gd=None, tan=None, cotan=None, device=None):
        ...
        self.__gd = torch.zeros(self.__numel_gd, requires_grad=True, device=self.__device)
        if isinstance(gd, torch.Tensor):
            self.fill_gd(gd.requires_grad_(), copy=False)

        self.__tan = torch.zeros(self.__numel_gd, requires_grad=True, device=self.__device)
        if isinstance(tan, torch.Tensor):
            self.fill_tan(tan.requires_grad_(), copy=False)

        self.__cotan = torch.zeros(self.__numel_gd, requires_grad=True, device=self.__device)
