...
def indices2coords(indices, shape, pixel_size=torch.tensor([1., 1.])):
    ...
    return torch.cat(
        [(pixel_size[0] * indices[:, 0]).view(-1, 1), (pixel_size[1] * (shape[1] - indices[:, 1] - 1)).view(-1, 1)], 1)
...
def blocks_to_2d(M):
    ...
    return torch.cat(
        [torch.cat([M.transpose(1, 2)[i::N] for i in range(N)], dim=1).transpose(1, 2)[i] for i in range(N)])
...
def kronecker(m1, m2):
    return torch.ger(m1.view(-1), m2.view(-1)).reshape(*(m1.size() + m2.size())).permute([0, 2, 1, 3]).reshape(
        m1.size(0) * m2.size(0), m1.size(1) * m2.size(1))
